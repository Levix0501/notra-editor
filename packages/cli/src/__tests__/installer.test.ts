...
import { generateInstallCommand, installDependencies } from '../utils/installer';
...
describe('Installer Unit Tests', () => {
  describe('generateInstallCommand', () => {
    ...
    test('should generate correct command for single dependency', () => {
      const deps: RegistryDependency[] = [{ name: '@tiptap/react', version: '2.4.0' }];
    ...
    test('should generate correct command for multiple dependencies', () => {
      ...
      const result = generateInstallCommand(deps);
      expect(result).toBe('pnpm add @tiptap/react@2.4.0 @tiptap/core@2.4.0 @tiptap/pm@2.4.0');
    });
  });
      ...
      fc.property(
        ...
        (dependencies) => {
          ...
          if (dependencies.length > 0) {
            const packagesInCommand = command.replace('pnpm add ', '').split(' ');
