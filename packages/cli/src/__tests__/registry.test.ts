...
import type { Registry, RegistryFile, RegistryDependency } from '../registry/types';

/**
 * Arbitrary for generating valid RegistryFile objects.
 */
const registryFileArbitrary: fc.Arbitrary<RegistryFile> = fc.record({
  path: fc.string({ minLength: 1, maxLength: 100 }).filter(s => !s.includes('\0')),
...
const registryDependencyArbitrary: fc.Arbitrary<RegistryDependency> = fc.record({
  ...
  version: fc.string({ minLength: 1, maxLength: 20 }).filter(s => !s.includes('\0')),
});
...
const registryArbitrary: fc.Arbitrary<Registry> = fc.record({
  ...
  dependencies: fc.array(registryDependencyArbitrary, { minLength: 0, maxLength: 10 }),
});
